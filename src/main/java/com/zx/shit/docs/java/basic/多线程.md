...
2.就绪(ready)：就绪状态说明有资格运行，调度程序未选择时，永远处于就绪状态，
    ...
    ▪ 锁池线程拿到对象锁后，也会进入就绪状态。
2.运行(running)：Java中将就绪(ready)线程调用start方法后进入就绪状态 和运行中(running)两种状态统称为"运行"。线程对象创建后，其他线程(main)线程调用了线程对象的start()方法。
    ▪ 线程调度程序从可运行池中选择一个线程作为当前线程时线程所处的状态，这也是线程进入运行状态的唯一一种方式。
    ▪ 该状态的线程位于可运行的线程中，等待被线程调度选中，获取CPU的使用权，此时处于就绪状(ready)状态。
      就绪状态的线程在获取CPU的时间片后变为运行中的状态(running)。
3.阻塞（blocked）：表示线程阻塞于锁。
    ▪ 阻塞状态是指线程阻塞在进入synchronized关键字修饰的方法或代码块(获取锁)的状态。
4.等待（waiting）：进入该状态的线程需要等其他线程做出一些动作（通知或中断）。
    ▪ 处于这种状态的线程不会被CPU分配执行时间，需要被显式的唤醒，否则处于无限等待的状态。
5.超时等待（timed_waiting）:该状态不同于waiting，它可以在指定时间后运行。
...
锁标志位为(0,1)当线程再次访问锁时，会判断线程id是否与threadId一致，一致则可以使用此对象，不一致时，sync升级偏量锁为轻量锁，通过自旋环一定次数来获取锁。
执行一定次数后，如果还没获取到对象。那么锁就会从轻量级升级为重量级锁。也就是互斥锁，锁标志位为(1,1)这个过程就是sync的锁升级。
