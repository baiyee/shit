...
###20.1Spring中的@Transaction什么时候会失效？
1.Spring事务是基于注解实现的，加了@Transaction注解，且方法修饰符不是private时会生效，没加不会生效。
2.当方法重载时，也就是子类继承父类，使用父类的@Transaction注解的方法，此时事务也不会生效。

###21.Spring事务的实现方式和实现原理？
1.Spring中的事务是基于数据库事务和AOP机制
2.对于使用@Transaction注解的Bean，Spring会创建一个代理对象作为Bean。
3.当调用该代理对象的方法时，会判断是否有加@Transaction注解，有加利用事务管理器创建一个数据库连接。
4.修改数据库连接的autocommit属性为false，禁止此连接自动提交，这是Spring事务中非常重要的异步。
5.执行当前方法，方法中会执行SQL.
6.通过事务的传播特性，来判断执行哪种事务，如果是required 没有事务创建事务，有事务加入事务，执行sql，执行sql代码没有抛出异常时，提交事务。
7.代码抛出异常时，会自动调用rollback方法回滚事务。
8.Spring的隔离级别对应数据库的隔离级别。
9.Spring的传播机制是自己实现的，底层基于数据库连接来实现。一个事务连接一个数据库，如果需要多个事务，如果传播配置设置需要新开一个事务，需要新建一个数据库连接再执行SQL。
